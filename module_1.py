        ...
        for k in range(1, len(terms)):
            ...
            if like == 1:
                if vid not in popular:
                    popular[vid] = 0;
                popular[vid] += 1;          
    output1 = ("rated_uid_add_vid:",rated)
    f=open("rated_uid_added_vid.txt","w")
    print(output1,file = f)
    f.close()
    output2 = ("popular_like_vid:",popular)
    f=open("popular_like_vid.txt","w")
    ...
    for scenario in scenarios:
        ...
        for line in open(os.path.join(args.data, 'f%dte.%s.txt'%(fold, scenario))):
            ...
            for k in range(1, len(terms)):
                ...
                if like == 1:
                    likes.add(teids[vid]);
                    likes_video = ("Likes video:",likes)
                    f=open("likes_video.txt","w")
                    print(likes_video, file = f)
                    f.close()
                    print(likes_video)
            if len(likes) != 0:
                hits = [0] * interval;
                ...
                for k in range(interval):
                    tresults[k] += hits[k];
                tcount += len(likes);
                total_likes_video = ("Total Likes in videos:",tcount)
                f=open("total_likes_video.txt","w")
                print(total_likes_video, file = f)
                f.close()
